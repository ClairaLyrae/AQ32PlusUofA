...
void ubloxParseData()
{
    if (ubloxClass == 1)         // NAV
    {
        if (ubloxId == 2)        // NAV:POSLLH
        {
            ...
            gps.height  = (float)ubloxMessage.nav_posllh.height * 0.01f;            // Meters
        }
        ...
        {
            gps.heading = (float)ubloxMessage.nav_velned.heading * 0.01f * D2R;    // Radians
            gps.gSpeed = (float)ubloxMessage.nav_velned.gSpeed  * 0.01f;          // Meters/Sec
        }
        else if (ubloxId == 33)  // NAV:TIMEUTC
        {
			gps.iTOW = (float)(ubloxMessage.nav_timeutc.hour * 10000 +
